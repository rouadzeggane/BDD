...
-- ====>>> Ce fichier sql s'appelle : B14-G1.sql
...
-->>>> ALTER SESSION SET >>>> dire que c de l'anglais ou de l'américain
PROMPT =========================================================
...
SELECT SUM((PUART*QTCOM) * (1-(REMISE/100))) AS chiffre_affaires FROM DETAILCOM ;
...
SELECT SUM((PUART*QTCOM) * (1-(REMISE/100))) AS chiffre_affaires,EXTRACT(DAY FROM DATCOM)AS DAY FROM DETAILCOM JOIN COMMANDES USING(NUMCOM)  GROUP BY EXTRACT(DAY FROM DATCOM);
...
SELECT SUM((PUART*QTCOM) * (1-(REMISE/100))) AS chiffre_affaires,EXTRACT(MONTH FROM DATCOM)AS MONTH FROM DETAILCOM JOIN COMMANDES USING(NUMCOM)  GROUP BY EXTRACT(MONTH FROM DATCOM);
...
SELECT SUM((PUART*QTCOM) * (1-(REMISE/100))) AS chiffre_affaires,EXTRACT(YEAR FROM DATCOM)AS YEAR FROM DETAILCOM JOIN COMMANDES USING(NUMCOM)  GROUP BY EXTRACT(YEAR FROM DATCOM);
...
SELECT c.VILCLI, SUM((d.PUART*d.QTCOM) * (1-(d.REMISE/100))) AS chiffre_affaires FROM DETAILCOM d, CLIENTS c, COMMANDES k WHERE d.NUMCOM  = k.NUMCOM AND c.CODCLI = k.CODCLI GROUP BY c.VILCLI;
...
SELECT c.PAYSCLI,c.VILCLI, SUM((d.PUART*d.QTCOM) * (1-(d.REMISE/100))) AS chiffre_affaires FROM DETAILCOM d, CLIENTS c, COMMANDES k WHERE d.NUMCOM  = k.NUMCOM AND c.CODCLI = k.CODCLI GROUP BY c.PAYSCLI,c.VILCLI;
...
PROMPT =====================================================================
PROMPT procedure CREERMAILMAG qui permet d Insérer les nouvelles données
PROMPT =====================================================================
PROMPT
CREATE OR REPLACE PROCEDURE CREERMAILMAG AS
BEGIN
    FOR LIGNE IN (SELECT NUMMAG , LOWER('bb'||NUMMAG||'@gmail.com') AS EMAIL FROM MAGASINS WHERE MAILMAG IS NULL) LOOP
    UPDATE MAGASINS SET MAILMAG = LIGNE.EMAIL WHERE NUMMAG = LIGNE.NUMMAG;
    END LOOP;
END;
/
...
WHERE TELCLI IS NULL OR NOT REGEXP_LIKE(TELCLI,'[^[:alpha:] ]');
SELECT * FROM CLISANSTEL;



-- Afficher les clients pour lesquels les mails sont invalides ou qui n'ont pas de mail (Code, Nom, Mail)
CREATE OR REPLACE VIEW CLIINVALIDMAIL (Code, Nom, Mail) AS SELECT CODCLI, NOMCLI, MAILCLI FROM CLIENTS WHERE MAILCLI IS NULL OR NOT REGEXP_LIKE (MAILCLI, '^[a-zA-Z0-9._%-]+@[a-zA-Z0-9._%-]+\.[a-zA-Z]{2,4}$');
...
AS SELECT CODCLI, NOMCLI, TELCLI FROM CLIENTS  
WHERE TELCLI IS NULL OR NOT REGEXP_LIKE(TELCLI,'^(0|(\+33|0033)|(\+21|00216)|(\+32|0032))[1-9][0-9]{8}') ;
...
WHERE  REGEXP_LIKE(NOMCLI,'[^[:alpha:] ]')OR  REGEXP_LIKE(PRENCLI,'[^[:alpha:] ]') OR NOMCLI <> UPPER(NOMCLI)OR NOMCLI<> UPPER(LEFT(NOMCLI,1))+LOWER(SUBSTRING(NOMCLI,2)));
...
--====================================================================================
--====================================================================================
CREATE TABLE METAREGULAREXPRESS
(
CATEGORY 					VARCHAR2(20), 
REGULAREXPR 					VARCHAR2(200),
CONSTRAINT PK_METAREGULAREXPRESS		PRIMARY KEY(CATEGORY),
CONSTRAINT CK_METAREGULAREXPRESS_CATEGORY	CHECK(CATEGORY = UPPER(CATEGORY)
);
...
CREATE OR REPLACE PROCEDURE TESTREG(TAB IN VARCHAR, COL IN VARCHAR) IS
REQ    VARCHAR2(200);
M     VARCHAR2(200);
Z     VARCHAR(200);
...
BEGIN
	...
	IF (COL LIKE ('TEL%')) THEN
	OPEN C1 FOR REQ ;
		LOOP
			FETCH C1 INTO M;
			EXIT WHEN C1%NOTFOUND;
SELECT REGULAREXPR INTO Z FROM REGULAREXPRES WHERE CATEGORY = 'TELFR-I'; --OR CATEGORY = 'TELFR' OR CATEGORY = 'TELBE-I' OR CATEGORY = 'TELBE' OR CATEGORY = 'TELTN-I' OR CATEGORY = 'TELTN'
			IF (REGEXP_LIKE ( M , Z ) )THEN
				dbms_output.put_line(M);
			END IF;
		END LOOP;
	CLOSE C1;
	ELSIF  (COL LIKE ('MAIL%')) THEN
	OPEN C1 FOR REQ ;
		LOOP
			FETCH C1 INTO M;
			EXIT WHEN C1%NOTFOUND;
			SELECT REGULAREXPR INTO Z FROM REGULAREXPRES WHERE CATEGORY='MAIL';
			IF (REGEXP_LIKE ( M , Z ) )THEN
				dbms_output.put_line(M);
			END IF;
		END LOOP;
...
EXEC TESTREG('CLIENTS','TELCLI');
EXEC TESTREG('CLIENTS','MAILCLI');
...
INSERT INTO TABCLI VALUES ('2994570', 'Madame', 'RAHMA', 'CLEMENCE', '3');
INSERT INTO TABCLI VALUES ('2994570', 'Madame', 'RAHMA', 'CLEMENCE', '3');
INSERT INTO TABCLI VALUES ('2994570', 'Madame', 'RAHMA', 'CLEMENSE', '3');
COMMIT; 
...
--COLONNE 1
CREATE OR REPLACE VIEW TEST(C1,C2,C3,C4,C5,COUNT,CONC,CONC4,CONC3,CONC4B) AS
	SELECT COL1,COL2,COL3,COL4,COL5,COUNT(*),CONCAT(CONCAT(CONCAT(COL1,COL2),CONCAT(COL3,COL4)),COL5) AS CONC ,
	CONCAT(CONCAT(COL2,COL3),CONCAT(COL4,COL5)) AS 	CONC4,
	CONCAT(CONCAT(COL2,COL4),COL5) AS CONC3,
	CONCAT(CONCAT(COL2,COL3),COL5) AS CONC4B
	FROM TABCLI
	GROUP BY COL1,COL2,COL3,COL4,COL5
	ORDER BY COUNT(*) DESC;

UPDATE TABCLI SET
	COL1=(SELECT C1 FROM TEST WHERE (CONCAT(CONCAT(COL2,COL3),CONCAT(COL4,COL5))=CONC4 OR (CONCAT(CONCAT(COL2,COL4),COL5)=CONC3 AND UTL_MATCH.EDIT_DISTANCE_SIMILARITY(COL3, C3)>=50) OR(CONCAT(CONCAT(COL2,COL3),COL5)=CONC4B AND UTL_MATCH.EDIT_DISTANCE_SIMILARITY(COL4, C4)>=50)) AND ROWNUM <=1) ;


--COLONNE 2
CREATE OR REPLACE VIEW TEST(C1,C2,C3,C4,C5,COUNT,CONC,CONC4,CONC3,CONC4B) AS
	SELECT COL1,COL2,COL3,COL4,COL5,COUNT(*),CONCAT(CONCAT(CONCAT(COL1,COL2),CONCAT(COL3,COL4)),COL5) AS CONC ,
	CONCAT(CONCAT(COL1,COL3),CONCAT(COL4,COL5)) AS CONC4,
	CONCAT(CONCAT(COL1,COL4),COL5) AS CONC3, CONCAT(CONCAT(COL1,COL3),COL5) AS CONC4B
	FROM TABCLI
	GROUP BY COL1,COL2,COL3,COL4,COL5
	ORDER BY COUNT(*) DESC;
...
--COLONNE 3
CREATE OR REPLACE VIEW TEST(C1,C2,C3,C4,C5,COUNT,CONC,CONC4,CONC4B) AS
	SELECT COL1,COL2,COL3,COL4,COL5,COUNT(*),CONCAT(CONCAT(CONCAT(COL1,COL2),CONCAT(COL3,COL4)),COL5) AS CONC ,
	CONCAT(CONCAT(COL1,COL2),CONCAT(COL4,COL5)) AS CONC4, CONCAT(CONCAT(COL1,COL2),COL5) AS CONC4B
	FROM TABCLI
	GROUP BY COL1,COL2,COL3,COL4,COL5
	ORDER BY COUNT(*) DESC;
...
-- COLONNE 4
CREATE OR REPLACE VIEW TEST(C1,C2,C3,C4,C5,COUNT,CONC,CONC4,CONC3) AS
	SELECT COL1,COL2,COL3,COL4,COL5,COUNT(*),CONCAT(CONCAT(CONCAT(COL1,COL2),CONCAT(COL3,COL4)),COL5) AS CONC ,
	CONCAT(CONCAT(COL1,COL2),CONCAT(COL3,COL5)) AS CONC4,
	CONCAT(CONCAT(COL1,COL2),COL5) AS CONC3
	FROM TABCLI
	GROUP BY COL1,COL2,COL3,COL4,COL5
	ORDER BY COUNT(*) DESC;
...
CREATE OR REPLACE VIEW TEST(C1,C2,C3,C4,C5,COUNT,CONC,CONC4,CONC3,CONC4B) AS SELECT
	COL1,COL2,COL3,COL4,COL5,COUNT(*),CONCAT(CONCAT(CONCAT(COL1,COL2),CONCAT(COL3,COL4)),COL5) AS CONC ,
	CONCAT(CONCAT(COL1,COL2),CONCAT(COL3,COL4)) AS CONC4,
	CONCAT(CONCAT(COL1,COL2),COL4) AS CONC3,
	CONCAT(CONCAT(COL1,COL2),COL3) AS CONC4B
	FROM TABCLI
	GROUP BY COL1,COL2,COL3,COL4,COL5
	ORDER BY COUNT(*) DESC;
...
--SUPPRESSIONS DES LIGNES DOUBLES
DELETE FROM TABCLI
                WHERE ROWID IN (
                            SELECT ROWID
